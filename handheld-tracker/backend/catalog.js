...
import { DEVICES, KNOWN_CHIPS } from "./devices.js";
...
export function ensureSeeded(conn = load()) {
  ...
  const tx = conn.transaction(() => {
    for (const [chip, { profile, benchmark }] of Object.entries(KNOWN_CHIPS)) {
      upsertChipProfile(conn, chip, profile, benchmark, "seed");
    }
    for (const d of DEVICES) {
      upsertDevice(conn, {
        ...
        profile: KNOWN_CHIPS[d.chip]?.profile ?? null,
        source: "seed",
      });
    }
  });
